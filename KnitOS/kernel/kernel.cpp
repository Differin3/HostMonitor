...
static void boot_populate_user_bins(void) {
    ...
    extern char user_wget_start[], user_wget_end[];
    extern char user_curl_start[], user_curl_end[];
    static const struct {
        const char* path;
        char* start;
        char* end;
        ...
        { "/tmp/wget.elf",      user_wget_start,    user_wget_end },
        { "/tmp/curl.elf",      user_curl_start,    user_curl_end },
    };
...
extern "C" void kernel_main(uint32_t multiboot_info) {
    ...
    extern char user_curl_start[], user_curl_end[];
            ...
            } else if (alen >= 4 && arg[0]=='w'&&arg[1]=='g'&&arg[2]=='e'&&arg[3]=='t') {
                which = "wget";
                start = user_wget_start; end = user_wget_end; name = "wget";
            } else if (alen >= 4 && arg[0]=='c'&&arg[1]=='u'&&arg[2]=='r'&&arg[3]=='l') {
                which = "curl";
                start = user_curl_start; end = user_curl_end; name = "curl";
            }
            if (!which) {
                terminal_writestring("\nUsage: runelf hello|demo2|demo3|launcher|argtest|httpd|ksshd|sshd|ftop|wget|curl");
                flush_line(); return;
            }
