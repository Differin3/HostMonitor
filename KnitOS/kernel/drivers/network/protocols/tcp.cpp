...
int tcp_connect_wait(struct tcp_connection* conn, int timeout_ms) {
    ...
    for (int i = 0; i < attempts; i++) {
        nic_process_packets();
        tcp_process_timers();
        extern void net_drain_loopback(void);
        net_drain_loopback();
...
/* Немедленное закрытие без блокирующего ожидания FIN — используется при
   завершении задачи (task_exit → socket_close_by_pid), где блокировка
   невозможна: задача уже умирает, sched_switch внутри net_wait_ms
   переключает контекст уходящей задачи и портит планировщик. */
void tcp_close_immediate(struct tcp_connection* conn) {
    if (!conn) return;
    tcp_connection_close(conn);
}
