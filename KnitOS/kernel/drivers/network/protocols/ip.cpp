...
static int ip_output_ex(uint32_t dest_ip, uint8_t protocol, const void* payload,
    ...
    if (payload_size <= max_payload) {
        ...
        if (local) return net_deliver_local(ip_buffer, (size_t)ip_len);
    ...
    while (offset < payload_size) {
        ...
        if (ip_len < 0) return -1;
        int rc;
        if (local) rc = net_deliver_local(ip_buffer, (size_t)ip_len);
        else rc = ip_send_frame_ttl(nif, next_hop, ip_buffer, (size_t)ip_len, ttl);
        if (rc != 0) return -1;
        offset += chunk;
    }
    return 0;
