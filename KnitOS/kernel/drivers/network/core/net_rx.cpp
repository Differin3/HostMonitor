...
/* Очередь loopback-пакетов. Синхронный loopback (вызов net_handle_ipv4
   изнутри ip_output) приводит к переполнению стека: каждый уровень
   (SYN -> SYN-ACK -> ACK) добавляет ~3 КБ локальных буферов.
   Вместо этого кладём пакет в очередь и обрабатываем итеративно. */
#define LBQ_SIZE 8
static struct {
    uint8_t data[1600];
    size_t len;
} lbq[LBQ_SIZE];
static volatile uint32_t lbq_head = 0, lbq_tail = 0;

int net_deliver_local(const void* ip_pkt, size_t len) {
    if (len > 1600) return -1;
    uint32_t next = (lbq_head + 1) % LBQ_SIZE;
    if (next == lbq_tail) return -1; /* переполнение */
    memcpy(lbq[lbq_head].data, ip_pkt, len);
    lbq[lbq_head].len = len;
    lbq_head = next;
    return 0;
}

void net_drain_loopback(void) {
    while (lbq_tail != lbq_head) {
        size_t len = lbq[lbq_tail].len;
        uint8_t tmp[1600];
        memcpy(tmp, lbq[lbq_tail].data, len);
        lbq_tail = (lbq_tail + 1) % LBQ_SIZE;
        net_handle_ipv4(netif_default(), 0, tmp, len);
    }
}
